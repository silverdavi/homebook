 ...
 * Fraction division.
 *
 * Pedagogy: dividing by a fraction is the same as multiplying by its
 * reciprocal — flip the second fraction, then multiply.
 */
...
function genProblem(level: number): FractionProblem {
  ...
  return {
    op: "divide",
    why: "Dividing fractions is multiplying by the upside-down second fraction. Flip, multiply, simplify.",
    equation: [
      { kind: "fraction", id: "x", n: a, d: b, tone: "violet" },
      { kind: "operator", id: "div", text: "÷" },
    ...
    steps: [
      {
        ...
        lines: [
          `Flip ${cN}/${d} to **${d}/${cN}** and change ÷ to ×.`,
          `Multiply numerators: **${a} × ${d} = ${rawN}**.`,
          `Multiply denominators: **${b} × ${cN} = ${rawD}**.`,
          g > 1
...
const op: FractionOpModule = {
  ...
  label: "Divide",
  symbol: "÷",
  blurb: "Divide one fraction by another",
  levels: LEVELS,
  genProblem,
};
