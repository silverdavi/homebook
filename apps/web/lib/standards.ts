 ...
 * Maps topics and subtopics to CCSS standards for frontend display.
 */
...
export const STANDARDS: Record<string, Standard> = {
  ...
  "6.RP.A.3": {
    ...
    description:
      "Use ratio and rate reasoning to solve real-world problems, including percent problems.",
    grade: "6",
  },
  "7.RP.A.3": {
...
/**
 * Mapping of subtopics to their applicable standards
 */
export const SUBTOPIC_STANDARDS: Record<string, string[]> = {
  // Fractions
...
export function getStandardsForSubtopic(subtopicId: string): Standard[] {
  const codes = SUBTOPIC_STANDARDS[subtopicId] || [];
 ...
 */
export function getStandardsForWorksheet(subtopicIds: string[]): Standard[] {
  const seen = new Set<string>();
  const standards: Standard[] = [];

  for (const subtopic of subtopicIds) {
    for (const std of getStandardsForSubtopic(subtopic)) {
...
/**
 * Format standards for display (e.g., in worksheet footer).
 */
export function formatStandardsForDisplay(standards: Standard[]): string {
