...
import { ArrowLeft, Trophy, RotateCcw, Bug, Code2, CheckCircle2, XCircle, AlertTriangle, BookOpen, ChevronDown, ChevronUp } from "lucide-react";
...
export function DebugDetectiveGame() {
  ...
  const handleAnswer = useCallback((idx: number) => {
    ...
    if (!practiceMode) {
      ...
      else { if (streak > 0) sfxStreakLost(); sfxWrong(); }
      setScore(s => s + pts);
      setStreak(newStreak);
    ...
    setPhase("feedback");
  }, [challenge, selectedChoice, streak, maxStreak, adaptive, practiceMode]);
  ...
  }, [currentIdx, challenges.length, score, highScore, maxStreak, results, adaptive, practiceMode]);
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-4 max-w-lg mx-auto gap-6">
          ...
          <div className="w-full">
            <button
              ...
              className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-300 hover:bg-amber-500/20 transition-colors text-sm font-semibold"
            >
              <span>🤔 What is Debugging?</span>
            ...
            {showLearn && (
              <div className="mt-2 px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-left text-sm text-slate-300 space-y-2">
                <p>
                  <strong className="text-amber-300">Debugging</strong> means finding and fixing mistakes in instructions. It&apos;s like proofreading, but for computer code!
                </p>
                <p>
                  When code doesn&apos;t do what it should, there&apos;s a &quot;bug&quot; hiding in it. Your job as a detective is to find it!
                </p>
                <div className="bg-amber-500/10 rounded-lg px-3 py-2 text-xs">
                  <p className="font-semibold text-amber-300 mb-1">Real-world example:</p>
                  <p>🍪 If a cookie recipe says &quot;add 100 cups of sugar&quot; — that&apos;s a bug! It should probably say 1 cup.</p>
                  <p className="mt-1">🐛 The word &quot;bug&quot; comes from a real moth that got stuck in an old computer!</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-amber-950/30 to-slate-900 text-white flex flex-col">
          ...
          <div className="flex-1 flex flex-col items-center p-4 max-w-lg mx-auto gap-4">
            ...
            <div className="flex gap-3 w-full">
              <button onClick={() => startGame(1, true)} className="flex-1 py-3 rounded-xl font-bold bg-teal-600 hover:bg-teal-500 transition flex items-center justify-center gap-2"><BookOpen size={16} /> Practice Again</button>
              <button onClick={() => setPhase("menu")} className="flex-1 py-3 rounded-xl font-bold bg-white/10 hover:bg-white/20 transition text-center">Back to Menu</button>
            </div>
          </div>
        </div>
      );
